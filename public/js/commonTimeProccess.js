var CommonTimeProccess = function (_RGB)
{
	...
	function proccessTheTime()
	{
		...
		userList.forEach(function (_user) {if (_user.getWeight() == 2) twoAppear = true;});
		//開始找出所有時間
		...
		else
		{
			...
			var twoCount = 0;
			userList.forEach(function (_user) {if (_user.getWeight() == 2) twoCount++});
			if (twoCount > 1)
			{
				for (jj = section.length-1 ; jj >= 0 ; jj--)
				{
					if(section[jj].users.length < twoCount) section.splice(jj,1);
				}
			}
	...
	function continuousCheck()
	{
		...
		answerList.forEach(
			function (_answer)
			{
				var thisLength = _answer[0].users.length > userList.length ? userList.length : _answer[0].users.length; 
				var colorString = RGB[parseInt((thisLength / userList.length * RGB.length) - 1)];
